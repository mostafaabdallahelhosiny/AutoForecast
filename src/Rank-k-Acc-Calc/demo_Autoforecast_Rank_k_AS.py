...
def Get_Best_Model_Dataset(data_name, win_res_dir_name, data_type):
    ...
    return best_model_dataset_index_mse, arr_models[best_model_dataset_index_mape + 1], arr_models[best_model_dataset_index_smape + 1]
...
if __name__ == '__main__':
    ...
    for i in range(0, n_folds): # n_folds
        ...
        uv_dir_all = 'results_all_uv/' ## uv for uni-variate
        dir_uv_list = os.listdir(uv_dir_all)
        data_type = 'Uni-var' 
        
        
        '''
        ## Commented for Training Time Calculation
        ...
        AS_train_time_seconds = time.time() - start_time 
        print('AS Train Time: '+ str(AS_train_time_seconds))
        
        train_time_vec.append(AS_train_time_seconds) 
        ...
        for idx in range(0,len(k1_neighbors)):
            ...
            if k1_neighbors[idx] < len(train_folds_list_uv[i]):
                k1_neighbors[idx] = k1_neighbors[idx]
            elif len(train_folds_list_uv[i]) <= k1_neighbors[idx] < int(2*len(train_folds_list_uv[i])):
                k1_neighbors[idx] = k1_neighbors[idx] - len(train_folds_list_uv[i])
            else:
                k1_neighbors[idx] = k1_neighbors[idx] - int(2*len(train_folds_list_uv[i])) 
        ...
        for train_k1 in k1_neighbors:
            ...
            AS_best_model_index_mse, model_name_mape, model_name_smape = Get_Best_Model_Dataset(train_k1_data_name, uv_dir_1, 'Uni-var')
            ...
            res = sorted(range(len(test_Y_mse.iloc[test_data_idx,:-1])), key = lambda sub: test_Y_mse.iloc[test_data_idx,:-1][sub])[:K]
            print(res)
            print(AS_best_model_index_mse)
            
            if AS_best_model_index_mse in res: #np.argmin(predicted_win[j]):
                cnt += 1
                print(cnt)
            ...
            if test_data_idx  < len(test_folds_list_uv[i]):
                test_data_idx = test_data_idx
            elif len(test_folds_list_mv[i]) <= test_data_idx < int(2*len(test_folds_list_uv[i])):
                test_data_idx -= len(test_folds_list_uv[i])
            else:
                test_data_idx -=  int(2 * len(test_folds_list_uv[i]))
