...
def Global_Best (dir_list, data_list, data_type):
    ...
    time_GB += time.time() - start_time
    print('Time for GB is ' + str(time_GB))
    '''
...
if __name__ == '__main__':
    ...
    for i in range(0, n_folds): # n_folds
        ...
        uv_dir_all = 'results_all_uv/' ##'results_all_mv/'
        ...
        glob_best_model_index_mse = Global_Best(uv_dir_all, train_folds_list_uv[i], data_type) ##, glob_best_model_name_mape, glob_best_model_name_smape
        #print(glob_best_model_name_mape)
        #print(glob_best_model_name_smape)
        #print(glob_best_model_name_mse)
        print(glob_best_model_index_mse)
        
        GB_train_time_seconds = time.time() - start_time 
        print('GB Train Time: '+ str(GB_train_time_seconds))
        
        train_time_vec.append(GB_train_time_seconds) 
        ...
        for j in range(0, len(test_Y_mse)):
            ...
            if glob_best_model_index_mse in res: #np.argmin(predicted_win[j]):
                cnt += 1
