...
if __name__ == '__main__':
    ...
    for i in range(0, n_folds):
        ...
        AF_best_inference_run_time_seconds = []
        
        for test_idx in range(len(test_folds_list_uv[i])):
            
            start_time = time.time()
            
            test_X = meta_features_test.iloc[test_idx,:]
            ...
            best_model_dataset_index_mse_orig = results_vec_mse_1.index(min(results_vec_mse_1))
            
            
            minimum_mse = np.min(yhat_mse)
            best_model_dataset_index_mse = np.where(yhat_mse == minimum_mse)
            
             
            ## Not needed for Inference time 
            #results_vec_gen_mse.append(test_Y_mse.iloc[test_idx, best_model_dataset_index_mse])
            

            AF_best_inference_run_time_seconds.append(time.time() - start_time)
            print('AutoForecast Per Dataset General Inference Time: '+ str(AF_best_inference_run_time_seconds))  
            
      
      
        print(AF_best_inference_run_time_seconds)    
        break
