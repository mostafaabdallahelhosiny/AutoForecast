...
def Write_Performance_File(dataset_name, dir_name, model_name, lag, mse, mape, smape, train_time, infer_time):
    ...
    with open('results/'+ dir_name + '/' + dataset_name_no_dir +'/' + model_name + '_lag=' + str(lag)+ '.csv', 'w', ) as myfile:
...
if __name__ == '__main__':
    ...
    start_ind = int(lag[0] + 1)
    end_ind  = int(2 * lag[0])
    #start_ind = int(lag[0] + 1)
    print(start_ind)
    #end_ind  = int(2 * lag[0])
    print(end_ind)
    prediction_length = 3           ## Length of Prediction. Known also as "Forecasting Horizon"
    freq = "5min"                   ## Data Frequency (Kept Constant for all models)
    preprocess = ["Exp_Smoothing", "No"] ## Data Representation or Pre-Processing
    ...
    dir_datasets_name = 'data-types-remain/Multi_Variate_Real/'#'data-cleaned/MHSETS' #'data-cleaned/TCPD_datasets/'  #'data-cleaned/MHMSETS' #'data-cleaned/Adobe_Service_CPU_Memory_datasets_7days' #data-cleaned/Mustafa Prev time-series datasets
    ...
    for dir_name in dir_datasets_directories:
        ...
        dir_datasets_sorted = get_files_by_file_size(dir_datasets_name + dir_name, reverse=False) ## Sort files by file size from smallest to largest
        ...
        for file_name in dir_datasets_sorted:
            ...
            if not os.path.exists('results/'+ dir_name + '/' + file_name_no_dir):
                os.mkdir('results/'+ dir_name + '/' + file_name_no_dir)
            else:
                if len(os.listdir('results/'+ dir_name + '/' + file_name_no_dir)) > 160:  ## If the dataset has more than 322 models, continue
                    continue
                    ...
                    for prep in preprocess:
                        ...
                        for i in Prophet_param_list_1:
                            for j in Prophet_param_list_2:
                                ...
                                ## Writing the Model Performance File 
                                Write_Performance_File(file_name, dir_name, "Prophet_" + str(i) + '_' + str(j) + '_Preprocess=' + str(prep), lag_par, mse, mape, smape, train_run_time_seconds, inference_run_time_seconds)
                        ...
                        for i in GP_param_list_1:
                            for j in GP_param_list_2:
                                ...
                                ## Writing the Model Performance File
                                Write_Performance_File(file_name, dir_name, "GP_" + str(i) + '_' + str(j)+ '_Preprocess=' + str(prep), lag_par, mse, mape, smape, train_run_time_seconds, inference_run_time_seconds)
                        ...
                        for i in AR_param_list_1:
                            for j in AR_param_list_2:
                                ...
                                if prediction_length + int(lag_par) >= len(df):
                                    yhat = model_fit.predict(start= index[int(end_ind)], end= len(df), dynamic=True)
                                else:
                                    yhat = model_fit.predict(start= index[int(end_ind)], end= index[int(end_ind)+prediction_length-1], dynamic=True)
