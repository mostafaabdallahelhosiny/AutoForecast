...
if __name__ == '__main__':  
       ...
       start_ind = int(lag_par + 1)
       end_ind =   int(2 * lag_par) + 1
   
       ## (B) Read the datasets directory and Load Datasets
       dir_datasets_name = 'data-types-orig/Multi_Variate_Real/'
       ...
       for dir_name in dir_datasets_directories:
           ...
           if not os.path.exists('Meta-Features/Meta-Feat_third_win/Multi_Variate_Real_third_win/'+ dir_name):        
               os.mkdir('Meta-Features/Meta-Feat_third_win/Multi_Variate_Real_third_win/'+ dir_name)
         
         
           if '.DS_Store' in dir_name:
               continue
            
           dir_datasets_sorted = get_files_by_file_size(dir_datasets_name+dir_name, reverse=False) ## Sort files by file size from 
           ...
           for file_name in dir_datasets_sorted:
               ...
               #try:
                   ...
                   ## Include only required points in meta-features extraction
                   #df = df.iloc[0:Num_points+1,:]
                   ...
                   ## Start Time of Extracting the features
                   start_time = time.time()
                   ...
                   final_meta_df.to_csv('Meta-Features/Meta-Feat_third_win/Multi_Variate_Real_third_win/'+dir_name+'/'+file_name[file_name.rfind('/')+1:file_name.index('.csv')]+'.csv', index=False)
